...
function levelDisplay(level: string): number {
  const gwei = weiToGwei(hexToInt(level))
  return roundGwei(gwei) || 0
}

function toDisplayUSD(num: number): string {
  if (!num || num === 0) return '?'
...
const GasFees: React.FC<GasFeesProps> = ({ gasPrice, color }) => (
...
interface GasFeesMarketProps {
  gasPrice: number
  fees: any
  color: string
}

const GasFeesMarket: React.FC<GasFeesMarketProps> = ({ gasPrice, fees: { nextBaseFee, maxPriorityFeePerGas }, color }) => {
    ...
    return (
      <>
        <ClusterRow>
          ...
          >
            <div style={{ padding: '6px', color: !explorer && 'var(--outerspace05)' } as any}>
        ...
        <ClusterRow>
          {sampleOperations.map(({ label, estimates }: any, i: number) => {
