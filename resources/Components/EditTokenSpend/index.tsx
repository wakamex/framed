...
const isMax = (value: any): boolean => max.isEqualTo(value)

const getMode = (requestedAmount: BigNumber, amount: any): string => {
  if (requestedAmount.eq(amount)) return 'requested'
  return isMax(amount) ? 'unlimited' : 'custom'
}

const isValidInput = (value: any, decimals: number): boolean => {
...
const Details: React.FC<DetailsProps> = ({ address, name }) => {
...
const Description: React.FC<{ isRevoke: boolean }> = ({ isRevoke }) => (
  <ClusterRow>
    <ClusterValue>
...
const EditTokenSpend: React.FC<EditTokenSpendProps> = ({
  data,
  ...
  const toDecimal = (baseAmount: any): string => new BigNumber(baseAmount).shiftedBy(-1 * decimals).toString(10)
  const fromDecimal = (decimalAmount: any): string => new BigNumber(decimalAmount).shiftedBy(decimals).toString(10)
  ...
  const updateCustomAmount = (value: any, decimals: number) => {
    if (!value) {
      setCustom('0')
